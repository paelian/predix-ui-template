...
module.exports = function(gulp) {
  return function() {
    ...
    var configFile = gulp.src('config/cf-dev.json', srcOptions).pipe(rename('cf.json')).pipe(gulp.dest('server'));
    var manifestFile = gulp.src('config/manifest-dev.yml', srcOptions).pipe(rename('manifest.yml')).pipe(gulp.dest('./'));
    var pm2mainFile = gulp.src('config/pm2_main-dev.js', srcOptions).pipe(rename('pm2_main.js')).pipe(gulp.dest('./'));
    var pm2ecoFile = gulp.src('config/ecosystem-dev.config.js', srcOptions).pipe(rename('ecosystem.config.js')).pipe(gulp.dest('./'));
