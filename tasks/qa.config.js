...
module.exports = function(gulp) {
  return function() {
    ...
    var configFile = gulp.src('config/cf-qa.json').pipe(rename('cf.json')).pipe(gulp.dest('server'));
    var manifestFile = gulp.src('config/manifest-qa.yml').pipe(rename('manifest.yml')).pipe(gulp.dest('./'));
    var pm2mainFile = gulp.src('config/pm2_main-qa.js').pipe(rename('pm2_main.js')).pipe(gulp.dest('./'));
    var pm2ecoFile = gulp.src('config/ecosystem-qa.config.js').pipe(rename('ecosystem.config.js')).pipe(gulp.dest('./'));
